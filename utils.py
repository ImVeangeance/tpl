...
def parse_rules(rules_str: str, lambda_symbol: str) -> dict[str, list[str]]:
    ...
    for rule in rules_str.splitlines():
        ...
        right = right.strip()
        if not sep:
            raise WrongRule(f'В правиле {rule} отсутствует символ ->')
        if not left:
            raise WrongRule(f'В правиле {rule} отсутствует левая часть')
        if not right:
            raise WrongRule(f'В правиле {rule} отсутствует правая часть')
