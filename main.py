...
def build_canon_grammar(grammar: Grammar) -> Grammar:
    print_to_actions('{: ^200}'.format('Начато приведение грамматики в каноничный вид'))
    ...
    if grammar != grammar_before_delete_empty_rules_and_chains:
        ...
        print_to_actions('Пустых правил не обнаружено!')
    ...
    if grammar != grammar_before_remove_chain_rulse:
        ...
        print_to_actions('Цепных правил не обнаружено!')
    grammar = remove_empty_and_chains_rules(grammar)
    if grammar_before_delete_empty_rules_and_chains == grammar:
        print_to_actions('{: ^200}'.format('Грамматика поданная на вход изначально является канонической'))
    else:
        print_to_actions('{: ^200}'.format('Завершено приведение грамматики к каноничному виду\n'))
    return grammar
...
def holmsky(grammar: Grammar):
    print_to_actions('{: ^200}'.format('Начато приведение канонической грамматики к бинарной грамматике Хомского'))
    print(grammar)
    # step 1 - remove long rules (2 <)
    old_gr: Grammar.P = grammar.P
    grammar, rules, rules_count = grammar.remove_long_rules()
    if rules != old_gr:
        print_to_actions(f'Грамматика после удаления длинных правил:\n{grammar_to_str(grammar)}')
    else:
        print_to_actions('Длинных правил не обнаружено!')
    print(grammar)
    ...
    if grammar != old_gr:
        print_to_actions(f'Уберём ситуации, когда в правиле встречаются несколько терминалов')
        print_to_actions('Грамматика после изменения:\n' + grammar_to_str(grammar))
    else:
        print_to_actions('Повторений нескольких нетерминальных символов в правилах не обнаружено!')
    ...
    print_to_actions('{: ^200}'.format('Окончено приведение канонической грамматики к бинарной грамматике Хомского'))
...
if __name__ == '__main__':
    ...
    grammar = False
    update_start_sym_combobox()
    window.ui.action.connect(SIGNAL('triggered()'), save_result)
    ...
    grammar = window.ui.pushButton.connect(SIGNAL('clicked()'), calculate)
